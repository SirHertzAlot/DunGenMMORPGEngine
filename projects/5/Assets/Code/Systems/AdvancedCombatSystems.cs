...
namespace DunGen.ECS.Systems.Combat
{
    ...
    public partial class ActionResolutionSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            var actionQuery = SystemAPI.QueryBuilder()
                .WithAllRW<CombatComponent>()
                .WithAllRW<ActionQueueComponent>()
                .WithAllRW<ActionCostComponent>()
                .WithAllRW<CombatStatsComponent>()
                .Build();
            using var actionEntities = actionQuery.ToEntityArray(Unity.Collections.Allocator.Temp);
            using var combatArr = actionQuery.ToComponentDataArray<CombatComponent>(Unity.Collections.Allocator.Temp);
            using var actionsArr = actionQuery.ToComponentDataArray<ActionQueueComponent>(Unity.Collections.Allocator.Temp);
            using var costsArr = actionQuery.ToComponentDataArray<ActionCostComponent>(Unity.Collections.Allocator.Temp);
            using var statsArr = actionQuery.ToComponentDataArray<CombatStatsComponent>(Unity.Collections.Allocator.Temp);

            for (int _i = 0; _i < actionEntities.Length; _i++)
            {
                var entity = actionEntities[_i];
                var combat = combatArr[_i];
                var actions = actionsArr[_i];
                var costs = costsArr[_i];
                var stats = statsArr[_i];
                ...
                // Write back mutated structs
                EntityManager.SetComponentData(entity, combat);
                EntityManager.SetComponentData(entity, actions);
    ...
    public partial class TurnTransitionSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            foreach (var (turnQueueRW, roundRW) in SystemAPI.Query<RefRW<TurnQueueComponent>, RefRW<CombatRoundComponent>>())
            {
                ref var turnQueue = ref turnQueueRW.ValueRW;
                ref var round = ref roundRW.ValueRW;
