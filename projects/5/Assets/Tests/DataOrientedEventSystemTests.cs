...
namespace DunGen.Tests
{
    ...
    public class DataOrientedEventSystemTests
    {
        ...
        [Test]
        public void EventBus_SubscribeAll_ReceivesEveryPublishedEvent()
        {
            // Arrange
            var bus = EventBus.Instance;
            ...
            var unsubscribe = bus.SubscribeAll((evt, _) => log.RecordPublishedEvent(evt));

            // Act
            bus.Publish(new EntitySpawnedEventData
            {
                EventId = 1,
                FrameNumber = 7,
                Timestamp = 0.117f,
            ...
            unsubscribe();
            bus.Publish(new EntityDestroyedEventData
            {
                EventId = 2,
                FrameNumber = 8,
                Timestamp = 0.133f,
            ...
            // Assert
            Assert.AreEqual(1, log.GetEvents().Count, "Global event observer should record published events until unsubscribed");
            Assert.IsInstanceOf<EntitySpawnedEventData>(log.GetEvents()[0]);
        }
