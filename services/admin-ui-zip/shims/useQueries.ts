...
import { backendImpl, ExternalBlob } from '../backend';
...
// The admin panel is wired to the authoritative backend through the authenticated
// /admin REST surface (nginx injects X-Admin-Key). The actor is always ready; there
// is no offline mode any longer.
function useOfflineActor(): { actor: backendInterface | null; isReady: boolean; status: 'online' | 'offline' } {
  return {
    actor: backendImpl,
    isReady: true,
...
export function useSaveGrid() {
  ...
  return useMutation({
    mutationFn: async ({ id, cells }: { id: string; cells: CellConfig[][] }) => {
      if (!actor || !isReady) throw new Error('Backend not available');
