...
import { GraphTraversalEngine } from "./graphTraversalEngine.js";
...
export function generateWorldArtifact(request) {
    const definition = (request.definition ?? request.Definition ?? {});
    const execution = (request.execution ?? request.Execution ?? {});
    ...
    for (let i = 0; i < ecs.enemyCount; i++) {
        ...
        const firstEventData = traversal.events[0]?.data;
        const encounter = firstEventData?.encounter;
...
function generateTerrainMesh(width, height, seed) {
...
function buildNormalizedHeightfield(width, height, seed) {
    ...
    for (let octave = 0; octave < 4; octave++) {
        ...
        const noise = new Array(gridWidth * gridHeight);
...
function buildTriangles(width, height) {
    const triangles = new Array((width - 1) * (height - 1) * 6);
    let offset = 0;
...
function applyNormals(vertices, triangles) {
    ...
    for (const vertex of vertices) {
        const magnitude = Math.sqrt(vertex.normalX * vertex.normalX +
...
function normalizeEcs(definition, execution) {
    const rawEcs = (definition.ecs ?? definition.Ecs ?? {});
    const parsedYaml = parseConstraints(execution.constraintsYaml ?? execution.ConstraintsYaml ?? "");
    const yamlEcs = typeof parsedYaml.ecs === "object" && parsedYaml.ecs ? parsedYaml.ecs : parsedYaml;
    return {
...
function randomInt(rng, minInclusive, maxExclusive) {
